...
- Project Planner: отдельная страница `/project-planner`, mock/offline режим, опциональный GigaChat provider, уточняющие вопросы, генерация с допущениями, run history, preview, DOCX export и on-demand PPTX executive pitch export из сохранённого `ProjectReport`.
- Reference Packs для Project Planner: curated JSON packs как prompt-context only, loader/selector, CLI validate/install/list, read-only API visibility, UI upload/validate/install/replace/template и selection preview.
...
- Project Planner: усилены DOCX preview/export, PPTX layout, prompt skeleton, LLM JSON normalizer, fallback-поведение, budget resolver, hard constraint guardrails, roadmap deadline/start/horizon guardrails, milestone density guardrail, Gantt-like представление и типографика DOCX.
