...
const referencePackTemplate = {
  ...
  budget_notes: [
    "Примечания к бюджету фиксируют допущения без цен; финансовая оценка считается backend-каталогом."
  ]
};
...
function ReferencePackItem({ pack }: { pack: ReferencePackMetadata }) {
  return (
    <div className="planner-question">
      <b>{pack.pack_name}</b>
      <div className="text-muted small">
        версия: {pack.pack_version} · дата источника: {formatDate(pack.source_date)} · доверие:{" "}
        {pack.confidence}
      </div>
    </div>
  );
        ...
        {validation ? (
          <div className="planner-question mt-3">
            ...
            <div className="text-muted small">
              ...
              {formatDate(validation.item.source_date)} · доверие: {validation.item.confidence}
            </div>
            <div className="text-muted small">
              фактов: {validation.item.facts_count} · имя файла: {validation.suggested_filename}
            </div>
...
export default function ProjectPlannerPage() {
  ...
  const exportableRuns = runs.filter((run) => run.has_docx).length;

  return (
    <div className="planner-page">
      <section className="planner-hero">
        <div className="planner-hero__content">
          <div className="planner-hero__eyebrow">Студия создания отчёта</div>
          <div className="page-intro planner-hero__intro">
            ...
            <div className="planner-hero__copy">Форма → уточнения → отчёт → DOCX/PPTX.</div>
          </div>
          ...
          <section className="planner-stage-shell">
              ...
              <div>
                <div className="planner-stage-shell__title-row">
                  ...
                  <InfoHint
                    text={
                      selectedRun
                        ? "Финальная зона просмотра, проверки допущений и выгрузки итоговых документов."
                        : "Здесь появится собранный пакет: паспорт проекта, дорожная карта, ресурсы, команда и DOCX/PPTX."
                    }
            ...
            {selectedRun?.warnings.length ? (
              <div className="alert alert-warning">
                <b>Предупреждения:</b> {selectedRun.warnings.join(" ")}
              </div>
            ) : null}
            {selectedRun?.assumptions.length ? (
              <div className="alert alert-info">
                <b>Допущения:</b> {selectedRun.assumptions.join(" ")}
              </div>
            ) : null}
            ...
            {report ? (
              ...
              <div className="surface-panel empty-state planner-preview-empty">
                Предпросмотр появится после генерации или выбора запуска.
              </div>
            )}
          </section>
        </div>
