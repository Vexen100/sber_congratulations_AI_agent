...
class HolidayLoader:
    ...
    async def load_from_json(self, json_path: Path, overwrite: bool = False) -> int:
        ...
        for item in holidays_data:
            ...
            existing = await self._find_existing_holiday(item["date"], item["title"])
    ...
    async def _find_existing_holiday(self, date_str: str, title: str) -> Optional[Holiday]:
        """Находит существующий праздник по дате и названию"""
        result = await self.session.execute(
            select(Holiday).where(
                Holiday.date == dt.date.fromisoformat(date_str), Holiday.title == title
            )
        )
    ...
    async def get_holidays_by_category(self, category: str, year: int) -> List[Holiday]:
    ...
    async def get_holidays_in_range(self, start_date: dt.date, end_date: dt.date) -> List[Holiday]:
    ...
    async def delete_old_holidays(self, before_year: int) -> int:
        ...
        result = await self.session.execute(select(Holiday).where(Holiday.date < cutoff_date))
