...
- **Извлечение данных**: базовая CRM-модель (клиенты, контакты, история взаимодействий) + праздники.
...
- **Генерация контента**:
  - выбор тона/шаблона по типу события и данным профиля клиента,
...
- **VIP approval**: удалён (все поздравления ведут себя как «обычные», без ручного шага согласования перед отправкой).
- **Доставка**: базовый канал отправки в **outbox (файлы)** + лог доставки (идемпотентность).
...
- **Feedback loop**: на странице `Greetings` сохраняются `score` и при необходимости `notes`/`outcome`; `training_verdict` для дообучения выставляется автоматически по оценке (см. `docs/DECISIONS.md`, §9).
- **Dashboard metrics**: на главной странице видны счётчики enriched-клиентов и feedback-записей.
- **Pipeline funnel on dashboard**: на главной странице видно post-generation воронку по статусам отправки + индикаторы проблемных запусков/ошибок доставки.
...
Важно: для TLS может понадобиться корневой сертификат Минцифры. Краткая инструкция собрана в `docs/GIGACHAT_INTEGRATION.md`. В крайнем случае для демо можно временно отключить проверку сертификата: `GIGACHAT_VERIFY_SSL_CERTS=false`.
...
- **отправляет только те поздравления, у которых дата события = сегодня**

### (Опционально) RAG / few-shot по менеджерскому feedback

По умолчанию RAG выключен: проект работает офлайн и проходит тесты без тяжёлых зависимостей.

Чтобы включить few-shot примеры из векторной БД:
...
3) Дайте несколько feedback с `training_verdict=accepted` и оценкой 4–5 — они начнут попадать в векторную БД, и при LLM-режиме будут добавляться как примеры в промпт. В UI для поздравления появится `generation_source` (например, `llm_fewshot_2_examples`).
