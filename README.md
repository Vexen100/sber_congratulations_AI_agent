...
- **Постоянное улучшение**: менеджерский feedback по поздравлениям (оценка/комментарий + вердикт `training_verdict=accepted|rejected`) сохраняется в БД и может использоваться для дальнейшего анализа качества / отбора примеров.
...
- **Feedback loop**: менеджер может на странице `Greetings` сохранить `score/notes` и вердикт `accepted|rejected` для каждого поздравления.
